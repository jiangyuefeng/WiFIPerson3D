...
class CSIList(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mat_data = sio.loadmat(join(self.root_dir, "alphapose", '20210116', 'alphapose_resize', "_".join(
                self.data_list[index].split(" ")[:2])) + ".mat")
        sample_JHMs = mat_data['heatmap']
        ...
        Joint(mat_data['keypoints'],video_name,frame_number,pad=True)
        sample = {
            'csi': sample_csi,
            'JHMs': sample_JHMs,
            'box': sample_box,
    ...
    def get_list(self):
        for root, dirs, files in os.walk(join(self.root_dir, "video", "20210116")):
            print(root)
            for frame_index in range(1,33):
                frame_name = "2021-1-16_"+str(frame_index)
                name = frame_name +"_Timestamp.txt"
                frame_number = 0
                for line in open(join(root, name), 'r'):
                    if os.path.exists(
                            join(
                                self.root_dir, "alphapose",'20210116','alphapose_resize',
                                frame_name+ "_video_" + str(frame_number) + ".mat")):
                        self.data_list.append(
                            frame_name + '_video' + " " + str(frame_number) + " " + line.strip())
                    frame_number += 1
        csi = dict()
        for root, dirs, files in os.walk(join(self.root_dir, "csi", "20210116")):
            for csi_index in range(1,33):
                csi_name = str(csi_index) + "_scaled"
                name = str(csi_index) +".txt"
                time_list = [
                    line.strip() for line in open(join(root, name), 'r')
                ]
                pd.to_datetime(time_list)
                csi_data = sio.loadmat(
                    join(self.root_dir,"mat", '20210116', csi_name + ".mat"))['video']
                #取实部
                csi_data = np.real(csi_data)
                frame_name = "2021-1-16_"+ str(csi_index)
